...
class SF_Settings {
	...
	public function __construct () {
		...
		$this->_field_obj = new SF_Fields_Settings();
		...
		if ( is_admin() ) {
			...
			add_action( 'sf_settings_process_single_field', array( $this, 'maybe_correct_upload_field_ssl' ), 10, 3 );
		}
	} // End __construct()

	/**
	 * Register the Framework admin menu.
	 * @access  public
	 * @since	1.0
	 * @return  void
	 */
	public function register_settings_screen () {
		...
		$ct = wp_get_theme();
		$theme_name = apply_filters( 'sf_branding_menu_label', $ct->__get( 'Name' ) );
		$icon = apply_filters( 'sf_branding_icon', '' );

		// ssmod add_object_page ( __( 'Settings', 'sfwp-locale' ), esc_html( $theme_name ), 'edit_theme_options', 'sf', array( $this, 'settings_screen' ), $icon );
		add_menu_page ( __( 'Settings', 'sfwp-locale' ), esc_html( $theme_name ), 'edit_theme_options', 'sf', array( $this, 'settings_screen' ), $icon );
		$sf_settings_screen_hook = add_submenu_page( 'sf', esc_html( $theme_name ), __( 'Settings', 'sfwp-locale' ), 'edit_theme_options', 'sf', array( $this, 'settings_screen' ) ); // Default

		// Load validation and save logic for the settings screen.
		add_action( 'load-' . $sf_settings_screen_hook, array( $this, 'settings_screen_logic' ) );

		do_action( 'sf_settings_register_settings_screen' );
	...
	public function admin_notices () {
		...
		if ( isset( $_GET['page'] ) && 'sf' == $_GET['page'] && isset( $_GET['updated'] ) && 'true' == $_GET['updated'] ) {
			$notices['settings-updated'] = array( 'type' => 'updated', 'message' => __( 'Settings saved.', 'sfwp-locale' ) );
		}
	...
	/**
	 * Run logic on the Framework settings screen.
	 * @access  public
	 * @since	1.0
	 * @return  void
	 */
	public function settings_screen_logic () {
		if ( ! empty( $_POST ) && check_admin_referer( $this->_field_obj->__get( 'token' ) . '_nonce', $this->_field_obj->__get( 'token' ) . '_nonce' ) ) {
			...
			do_action_ref_array( 'sf_settings_save_before', $data, $this );

			$options_collection = (array)get_option( 'sf_options', array() );
			...
			if ( 0 < count( $data ) ) {
				foreach ( $data as $k => $v ) {
					...
					if ( ! current_user_can( 'unfiltered_html' ) && in_array( $k, sf_disabled_if_not_unfiltered_html_option_keys() ) ) {
						continue;
					}
				...
				update_option( 'sf_options', $options_collection );
			}

			do_action_ref_array( 'sf_settings_save_after', $data, $this );
	...
	public function get_settings_template () {
		return get_option( 'sf_template', array() );
	...
	private function _process_single_field ( $k, $v ) {
		...
		return apply_filters( 'sf_settings_process_single_field', $value, $k, $v );
