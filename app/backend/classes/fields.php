...
class SF_Fields {
	...
	public function __construct () {
		...
		$this->_assets_url = SF()->get_assets_url();
	...
	public function init ( $data = array() ) {
		if ( 0 >= count( $data ) ) return new WP_Error( 'bad_settings_data', __( 'The settings data provided is malformed. Please try again.', 'sfwp-locale' ) );
	...
	public function validate_fields ( $data, $section = '' ) {
		if ( ! is_array( $data ) || 0 >= count( $data ) ) return new WP_Error( 'bad_field_data', __( 'The provided field data is invalid and cannot be validated.', 'sfwp-locale' ) );
		...
		if ( 0 < count( $data ) ) {
			foreach ( $data as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true == (bool)apply_filters( 'sf_validate_field_' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'sf_validate_field_' . $fields[$k]['type'], array( $this, $method ), 10, 2 );
				}

				$method_output = apply_filters( 'sf_validate_field_' . $fields[$k]['type'], $v, $fields[$k] );
				// $method_output = apply_filters( 'sf_validate_field_' . $k, $v, $fields[$k] );
	...
	public function render_fields ( $data, $mode = 'table' ) {
		...
		foreach ( $data as $k => $v ) {
			...
			$html .= '<td' . $colspan . '><span class="sf-field sf-field-' . esc_attr( $v['type'] ) . '">' . $field . '</span></td>' . "\n";
			$html .= '</tr>' . "\n";
		}
		return $html;
	} // End render_fields()
	...
	public function render_single_field ( $key, $args ) {
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'sf_no_description_fields', array( 'checkbox', 'info' ) ) ) ) {
			if ( isset( $args['desc'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['desc'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'sf_newline_description_fields', array( 'textarea', 'select', 'select2', 'slider', 'images', 'info', 'border', 'typography', 'color', 'upload', 'calendar', 'timestamp', 'select_taxonomy', 'multi_field' ) ) ) ) {
	...
	protected function render_field_typography ( $key, $args ) {
		...
		/* Size in Pixels */
		$html .= '<select class="sf-typography sf-typography-size sf-typography-size-px hide-if-em" name="'. esc_attr( $key . '[size_px]' ) . '" id="'. esc_attr( $key . '_size' ) . '">' . "\n";
		for ( $i = 9; $i < floatval( apply_filters( 'sf_fields_typography_font_size_px_upper_limit', 71 ) ); $i++ ) {
			$html .= '<option value="'. esc_attr( $i ) .'" ' . selected( floatval( $value['size'] ), $i, false ) . '>'. esc_html( $i ) . '</option>' . "\n";
		}
		$html .= '</select>' . "\n";

		/* Size in EMs */
		$html .= '<select class="sf-typography sf-typography-size sf-typography-size-em hide-if-px" name="'. esc_attr( $key . '[size_em]' ) . '" id="'. esc_attr( $key . '_size' ) . '">' . "\n";
		$em = 0;
		for ( $i = 0; $i < 39; $i++ ) {
		...
		$html .= '<select class="sf-typography sf-typography-unit" name="'. esc_attr( $key .'[unit]' ) . '" id="'. esc_attr( $key . '_unit' ) . '">' . "\n";
		$html .= '<option value="px" ' . $px . '">px</option>' . "\n";
		$html .= '<option value="em" ' . $em . '>em</option>' . "\n";
		$html .= '</select>' . "\n";

		/* Weights */
		$font_weights = (array) apply_filters( 'sf_fields_typography_font_weights', array( '300' => __( 'Thin', 'sfwp-locale' ), '300 italic' => __( 'Thin Italic', 'sfwp-locale' ), 'normal' => __( 'Normal', 'sfwp-locale' ), 'italic' => __( 'Italic', 'sfwp-locale' ), 'bold' => __( 'Bold', 'sfwp-locale' ), 'bold italic' => __( 'Bold/Italic', 'sfwp-locale' ) ) );

		if ( 0 < count( $font_weights ) ) {
			$html .= '<select class="sf-typography sf-typography-font-weight sf-typography-style" name="'. esc_attr( $key . '[style]' ) . '" id="'. esc_attr( $key . '_style' ) . '">' . "\n";
		...
		/* Font Face */
		$font_faces = sf_get_system_fonts();
		$google_fonts = sf_get_google_fonts();
		if ( 0 < count( $google_fonts ) ) {
			$font_faces[''] = __( '-- Google WebFonts --', 'sfwp-locale' );
		...
		if ( 0 < count( $font_faces ) ) {
			$test_cases = sf_get_system_fonts_test_cases();

			$html .= '<select class="sf-typography sf-typography-font-face sf-typography-face" name="'. esc_attr( $key . '[face]' ) . '" id="'. esc_attr( $key . '_face' ) . '">' . "\n";
		...
		/* Border Color */
		$html .= '<input id="' . esc_attr( $key . '_color' ) . '" name="' . esc_attr( $key . '[color]' ) . '" size="40" type="text" class="sf-typography-color colour" value="' . esc_attr( $value['color'] ) . '" />' . "\n";
	...
	protected function render_field_border ( $key, $args ) {
		...
		/* Border Width */
		$html .= '<select class="sf-border sf-border-width" name="'. esc_attr( $key . '[width]' ) . '" id="'. esc_attr( $key . '_width' ) . '">' . "\n";
		for ( $i = 0; $i < intval( apply_filters( 'sf_fields_border_width_upper_limit', 21 ) ); $i++ ) {
			$html .= '<option value="' . esc_attr( $i ) . '" ' . selected( intval( $value['width'] ), $i, false ) . '>'. esc_html( $i ) . 'px</option>' . "\n";
		}
		$html .= '</select>' . "\n";

		/* Border Style */
		$border_styles = (array) apply_filters( 'sf_fields_border_styles', array( 'solid' => __( 'Solid', 'sfwp-locale' ), 'dashed' => __( 'Dashed', 'sfwp-locale' ), 'dotted' => __( 'Dotted', 'sfwp-locale' ) ) );

		if ( 0 < count( $border_styles ) ) {
			$html .= '<select class="sf-border sf-border-style" name="'. esc_attr( $key . '[style]' ) . '" id="'. esc_attr( $key . '_style' ) . '">' . "\n";
	...
	protected function render_field_info ( $key, $args ) {
		$html = '<div id="' . esc_attr( $key ) . '" class="sf-notice">' . "\n";
	...
	protected function render_field_masked_input ( $key, $args ) {
		...
		$html = '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" size="40" type="text" value="' . esc_attr( $this->get_value( $key, $args['std'] ) ) . '" class="sf-input-masked" data-placeholder="' . esc_attr( $placeholder ) . '" />' . "\n";
		return $html;
	...
	protected function render_field_calendar ( $key, $args ) {
		...
		$html .= '<input class="sf-input-calendar" type="text" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( date( 'm/d/Y', strtotime( $val ) ) ) . '">';
		return $html;
	...
	protected function render_field_upload ( $key, $args ) {
		...
		$placeholder = apply_filters( 'sf_placeholder_image_url', get_template_directory_uri() . '/app/backend/assets/images/placeholder.png' );
		$class = ' no-image';
		if ( '' != $url || 0 < intval( $id ) ) $class = ' has-image';

		$html = '<span class="upload-field">' . "\n";
		$html .= '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" size="34" type="text" class="input-upload" value="' . esc_attr( $url ) . '" /> <a href="#" class="button" data-uploader-title="' . esc_attr( sprintf( __( 'Select %s', 'sfwp-locale' ), $args['name'] ) ) . '" data-uploader-button-text="' . esc_attr( sprintf( __( 'Use image as %s', 'sfwp-locale' ), $args['name'] ) ) . '">' . __( 'Upload', 'sfwp-locale' ) . '</a>' . "\n";
		...
		$html .= '<a href="#" class="remove">' . sprintf( __( 'Remove %s', 'sfwp-locale' ), $args['name'] ) . '</a>' . "\n";
	...
	protected function render_field_upload_min ( $key, $args ) {
		...
		$html = '<span class="upload-field">' . "\n";
		$html .= '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" type="hidden" class="input-upload" value="' . esc_attr( $url ) . '" /> <a href="#" class="button" data-uploader-title="' . esc_attr( sprintf( __( 'Select %s', 'sfwp-locale' ), $args['name'] ) ) . '" data-uploader-button-text="' . esc_attr( sprintf( __( 'Use image as %s', 'sfwp-locale' ), $args['name'] ) ) . '">' . __( 'Upload', 'sfwp-locale' ) . '</a>' . "\n";
		$html .= '<input id="' . esc_attr( $key ) . '-id" name="' . esc_attr( $key ) . '-id" type="hidden" class="input-upload-id" value="' . esc_attr( $id ) . '" /> ' . "\n";
		$html .= '</span>' . "\n";
	...
	protected function render_field_timestamp ( $key, $args ) {
		...
		$html = '<input type="hidden" name="datepicker-image" value="' . esc_url( admin_url( 'images/date-button.gif' ) ) . '" />' . "\n";
		$html .= '<input class="sf-input-calendar" type="text" name="' . esc_attr( $key . '[date]' ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( date( 'm/d/Y', $val ) ) . '">';
		$html .= '<span class="time-selectors">' . "\n";
		$html .= ' <span class="sf-timestamp-at">' . __( '@', 'sfwp-locale' ) . '</span> ';

		$html .= '<select name="' . esc_attr( $key . '[hour]' ) . '" class="sf-select-timestamp">' . "\n";
			for ( $i = 0; $i <= 23; $i++ ) {
		...
		$html .= '<select name="' . $key . '[minute]" class="sf-select-timestamp">' . "\n";
			for ( $i = 0; $i <= 59; $i++ ) {
	...
	public function enqueue_styles () {
		...
		//wp_register_style( $this->_token . '-chosen', esc_url( $this->_assets_url . 'css/lib/chosen.css' ) ); - deprecated for cdn
		wp_register_style( $this->_token . '-chosen', '//cdnjs.cloudflare.com/ajax/libs/chosen/1.5.1/chosen' . $suffix . '.css', '1.5.1' );
	...
	public function enqueue_scripts () {
		...
		//wp_register_script( 'jquery-masked-input', esc_url( $this->_assets_url . 'js/lib/jquery-masked-input.js' ), array( 'jquery' ) ); - deprecated for cdn
		wp_register_script( 'jquery-masked-input', '//cdnjs.cloudflare.com/ajax/libs/jquery.maskedinput/1.4.1/jquery.maskedinput' . $suffix . '.js', array( 'jquery' ) );
		...
		//wp_register_script( $this->_token . '-chosen', esc_url( $this->_assets_url . 'js/lib/jquery-chosen.js' ), array( 'jquery' ) ); - deprecated for cdn
		wp_register_script( $this->_token . '-chosen', '//cdnjs.cloudflare.com/ajax/libs/chosen/1.5.1/chosen.jquery' . $suffix . '.js', array( 'jquery' ) );
		wp_register_script( $this->_token . '-chosen-rtl', esc_url( $this->_assets_url . 'js/chosen-rtl.js' ), array( $this->_token . '-chosen' ) );
	...
	public function init_fields ( $data ) {
		...
		$this->_fields = (array)apply_filters( 'sf_fields_init_fields', $this->_fields, $this->_fields );
		return $this->_fields;
	} // End init_fields()
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'sf_fields_supported_fields', array( 'text', 'checkbox', 'radio', 'textarea', 'multicheck', 'multicheck2', 'select', 'select2', 'upload', 'upload_min', 'upload_field_id', 'calendar', 'time', 'time_masked', 'timestamp', 'color', 'typography', 'border', 'images', 'info', 'slider', 'masked_input' , 'select_taxonomy', 'multi_field' ) );
	...
	public function get_value ( $key, $default ) {
		...
		if ( true == apply_filters( 'sf_use_theme_mods', false ) ) {
