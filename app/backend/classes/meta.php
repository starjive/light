...
class SF_Meta {
	...
	public function __construct () {
		...
		$this->_field_obj = new SF_Fields_Meta();
	...
	public function register_meta_boxes () {
		$defaults = array(
						'id' => 'sf-settings',
						'title' => sprintf( __( '%s Settings', 'sfwp-locale' ), wp_get_theme()->__get( 'Name' ) ),
		...
		$settings = apply_filters( 'sf_metabox_settings', $settings, $settings['page'], $settings['id'] );
		$meta_boxes = array( 'sf-settings' => $settings );

		$meta_boxes = (array)apply_filters( 'sf_meta_boxes', $meta_boxes );
	...
	public function meta_box_save ( $post_id ) {
		...
		if ( isset( $screen->base ) && 'nav-menus' != $screen->base ) {
			...
			foreach ( $key_value_pairs as $k => $v ) {
				...
				$field_key = $k;
				if ( true == (bool)apply_filters( 'sf_meta_use_underscore_prefix', false ) ) {
					$field_key = '_' . $k;
				}
	 ...
	 * @return  void
	 */
	public function maybe_assign_default_section ( $fields, $section = 'sf-settings' ) {
		if ( 0 < count( $fields ) ) {
	...
	/**
	 * Transform data to make sure it matches what SF_Fields is expecting.
	 * @access  public
	 * @since	1.0
	 * @return  void
	 */
	...
	public function get_settings_template () {
		return get_option( 'sf_custom_template', array() );
