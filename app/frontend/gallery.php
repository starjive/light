...
global $sf_options, $post, $post_settings;
	...
	$settings = apply_filters( 'sf_post_gallery_settings', $settings, $settings['post_id'] );
	...
	if ( ! empty( $images ) ) {
		...
		foreach ( $images as $k => $img ) {
			...
			if ( $img['caption'] != '' ) {
				$caption = apply_filters( 'sf_post_gallery_image_caption', '<span class="photo-caption">' . $img['caption'] . '</span>', $img['id'] );
			...
			$html .= $before . sf_image( 'noheight=true&return=true&src=' . $src . '&width=' . $settings['width'] ) . $after;
		}
