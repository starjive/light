...
if ( ! is_admin() ) {
	add_action( 'wp_enqueue_scripts', 'sf_load_woocommerce_css', 20 );
}

add_action( 'wp', 'sf_wc_disable_css' );

/**
 * Hook in on activation
 */
global $pagenow;
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) add_action( 'init', 'sf_install_theme', 1 );
...
add_action( 'woocommerce_before_main_content', 'sf_woocommerce_before_main_content', 10 );
add_action( 'woocommerce_after_main_content', 'sf_woocommerce_after_main_content', 20 );

/**
 * Related Products
 * Filters related products to adjust display
 */
add_filter( 'woocommerce_output_related_products_args', 'sf_woocommerce_output_related_products' );
...
/**
 * Breadcrumbs
 * Remove the WooCommerce breadcrumb. The Framework breadcrumb is hooked in later.
 */
remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
add_action( 'woocommerce_before_main_content', 'woocommercesf_breadcrumb', 20, 0 );
...
add_action( 'sf_main_after', 'woocommerce_get_sidebar', 10 );
...
add_action( 'woocommerce_after_main_content', 'sf_woocommerce_after_main_content', 01, 0 );
 ...
 */
add_filter( 'get_product_search_form', 'sf_custom_wc_search' );

/**
 * Header Cart
 * Optionally display a header cart link next to the navigation menu.
 */
add_action( 'sf_nav_inside', 'sf_add_nav_cart_link', 20);
 ...
 */
add_filter( 'woocommerce_placeholder_img_src', 'sf_wc_placeholder_img_src' );
