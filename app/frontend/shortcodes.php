...
/**
 * 15. Link (with optional affiliate link)
 *
 * This function produces link back to us, with an affiliate link if you've
 * specified one in the Framework.
 *
...
function sf_shortcode_footer_sf_link ( $atts ) {
	...
	$output = sprintf( '%s<a href="%s" alt="%s" title="%s">%s</a>%s', $atts['before'], esc_url( $sf_link ), 'WordPress Themes & Plugins' , 'WordPress Themes & Plugins', 'STARJIVESTUDIOS', $atts['after'] );
